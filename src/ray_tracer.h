...
private:
	...
	static void renderThread(RayTracer *rt, int index, int rowsPerThread, double ratio, double sampleSize);
	Vect trace(Ray& ray, int depth, int currentSteps, int *signsPrev);
	Vect traceShadow(Vect& origin, Vect& normal, int *signsPrev);
	double newton(Object* obj, Ray& ray, double t, double tol, int max_iter);
public:
	RayTracer(int width, int height, int fov, int numSamples, int numThreads, double rayStepSize, int maxRaySteps, int maxRayDepth, double rayTol, int maxIter);
	~RayTracer();
	Vect **render();
